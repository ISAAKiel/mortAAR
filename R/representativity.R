...
#' life table
#'
#' Weiss 1973 and Bocquet-Appel and Masset 1978 have devised indices
#' which check if the non-adult age groups are represented in proportions
#' as can be expected from modern comparable data. Whether this is really
#' applicable to archaeological data-sets is a matter of debate.
#'
...
lt.representativity <- function(life_table) {
  ...
  weiss_i1 <-  if (indx$mortality$q10_5 < indx$mortality$q15_5) {
    representativity_sum <- representativity_sum + 0
    crayon::green(paste0("5q10 = ", round(indx$mortality$q10_5, 1), " < 5q15 = ",
                         round(indx$mortality$q15_5, 1),"."))
  } else {
    representativity_sum <- representativity_sum + 1
    crayon::red(paste0("5q10 = ", round(indx$mortality$q10_5, 1), " > 5q15 = ",
                       round(indx$mortality$q15_5, 1),"."))
  }

  weiss_i2 <- if (indx$mortality$q0_5 > indx$mortality$q15_5) {
    representativity_sum <- representativity_sum + 0
    crayon::green(paste0("5q0 = ", round(indx$mortality$q0_5, 1), " > 5q15 = ",
                         round(indx$mortality$q15_5, 1),"."))
  } else {
    representativity_sum <- representativity_sum + 1
    crayon::red(paste0("5q0 = ", round(indx$mortality$q0_5, 1),
                       " < 5q15 = ", round(indx$mortality$q15_5, 1),"."))
  }


  # Criteria for representativity after Bocquet-Appel and Masset

  child_i <- if (indx$child_i$d5_9_d10_14 < 2) {
    representativity_sum <- representativity_sum + 1
    crayon::red(paste0("Index of children is ", round(indx$child_i$d5_9_d10_14, 2),
                       " and therefore under the expected value of 2."))
  } else {
    representativity_sum <- representativity_sum + 0
    crayon::green(paste0("Index of children is ", round(indx$child_i$d5_9_d10_14, 2),
                         " and therefore above the expected value of 2."))
  }

  juvenile_i <- if (indx$juvenile_i$d5_14_d20plus < 0.1) {
    representativity_sum <- representativity_sum + 1
    crayon::red(paste0("Index of juveniles is ", round(indx$juvenile_i$d5_14_d20plus,2),
                       " and therefore under the expected value of 0.1."))
  } else {
    representativity_sum <- representativity_sum + 0
    crayon::green(paste0("Index of juveniles is ", round(indx$juvenile_i$d5_14_d20plus,2),
